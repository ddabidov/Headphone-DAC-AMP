...
Debugging considerations should have been a higher priority during the PCB design process. The inclusion of dedicated debug LEDs would have provided immediate visual feedback on system operation, allowing faults to be identified quickly and localized without extensive probing. Similarly, the addition of test points on critical signals would have significantly improved accessibility during debugging, particularly for issues that were not immediately apparent. Providing headers for each power source would have simplified the diagnosis of power-related issues and enabled the use of external power supplies during testing. This would have allowed individual power domains to be isolated and verified independently, improving both safety and efficiency during troubleshooting. Footprint verification should also have received greater attention. An incorrect footprint was used for the 0.9 V LDO, resulting in a PCB that could not physically accommodate the selected component. This error rendered the affected section of the board unusable and could have been prevented by more thorough cross-checking of datasheets prior to fabrication. Verifying all component footprints before ordering the PCB is a critical step that should not be overlooked.Another oversight was the configuration of copper pours. In particular, the VBUS copper pour on the USB-C connector was incorrectly configured, resulting in only a minimal electrical connection between VBUS and the rest of the board. This significantly compromised power delivery and functionality. Ensuring that all pours are correctly assigned, connected, and verified through design rule checks and visual inspection is essential prior to fabrication. <br>
...
## 9. Future Work
As mentioned previously, several lessons learned highlight areas of future work that must be revisited or newly implemented. At present, the 0.9 V LDO is nonfunctional, and the XMOS processor is not operating as intended. When the XTAG debugger is connected between the board and a computer, no device is detected, indicating a critical issue that must be resolved before further system validation can occur. In addition, the functionality of the analog filtering stages and the amplifier output has not yet been verified. Comprehensive testing of these subsystems will be required once the XMOS device is operational. Further improvements include the implementation of additional debugging features, such as status LEDs and accessible test points, to aid in troubleshooting and validation. Layout-related changes are also necessary, including updates to component footprints, copper pours, and trace widths to better align with electrical and manufacturing requirements. These modifications will help improve signal integrity, power delivery, and overall board reliability in future revisions.
